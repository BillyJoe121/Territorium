...
1. Consultar el estado de la tarea en `expediente_tasks` o `job_attempts` mediante el ID de la tarea. Revisar los eventos `expediente_worker_wake_accepted`, `expediente_worker_wake_rejected` o `expediente_worker_wake_failed` en los logs de la Edge Function.
...
### 2.2 Configuración de activación del worker
1. Generar un secreto aleatorio de al menos 32 caracteres y cargarlo como `WORKER_WAKE_TOKEN` tanto en el worker como en los secretos de la Edge Function.
2. Configurar en la Edge Function `WORKER_WAKE_URL` con la URL HTTPS exacta `https://<worker>/internal/wake`.
3. Desplegar ambos servicios y confirmar que `/ready` responde `200` antes de la primera carga.
4. Nunca poner `WORKER_WAKE_TOKEN` ni `WORKER_WAKE_URL` en variables `VITE_*`, en el navegador, en el repositorio o en los logs.

### 2.3 Fallo Transitorio o Indisponibilidad del Proveedor de IA
...
### 2.4 Alerta de Consumo Anómalo de Tokens o Costo
...
- [ ] `WORKER_WAKE_URL` y el mismo `WORKER_WAKE_TOKEN` están configurados solo en la Edge Function y el worker; una carga de prueba avanza desde `queued`.
